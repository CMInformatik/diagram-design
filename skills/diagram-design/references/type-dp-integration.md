...
focal_accent: "#009fe3"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(29,56,73,0.05)`, stroke `rgba(29,56,73,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(79,93,117,0.06)`, stroke `#7a8e9b`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#009fe3` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#4a6272` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#1a808c` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#4a6272` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#009fe3` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#1a808c"/></marker>
...
- **Labels:** every `primary`, `secondary`, `federated`, `auth` edge gets a protocol label (Roboto Mono 8 px, paper-filled rect mask with 6–10 px clear gap above the stroke). `trigger` edges are unlabelled.
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#f7f9fa` | `#1d3849` |
| Ink | `#1d3849` | `#f7f9fa` |
| Muted | `#4a6272` | `#b7c4ce` |
| Accent | `#009fe3` | `#33b5ec` |
| Link (federated) | `#1a808c` | `#4fb3be` |
| Side-column fill | `rgba(79,93,117,0.06)` | `rgba(247,249,250,0.06)` |
| Side-column stroke | `#7a8e9b` | `rgba(247,249,250,0.30)` |
| Zone fill | `rgba(29,56,73,0.025)` | `rgba(247,249,250,0.04)` |
| Zone stroke | `rgba(29,56,73,0.32)` | `rgba(247,249,250,0.30)` |
| Non-focal bar fill | `rgba(29,56,73,0.05)` | `rgba(247,249,250,0.06)` |
