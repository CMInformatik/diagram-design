...
- **Labels sit inside the cell**, top-left, 16px in from the edge: name in Arial 12–14px 600, value in Roboto Mono 9px on the next line. Three tiers by cell size:
...
- **Plot area:** the treemap's — `x` 40 → 956, `y` 40 → 420 inside a `0 0 1000 500` viewBox, legend block at `y=462`/`478`/`488`. Column captions sit under the plot at `y=438`, Roboto Mono 9px, centred on their column. The shipped example spends 900px of column width across five columns with four 4px gutters.
